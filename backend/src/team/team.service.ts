...
export class TeamService {
  ...
  async listTeamMembers(currentUser: any) {
    ...
    console.log(`[TeamService] Tenant ${tenantId}: ${result.data.length} usuários. OwnerId: ${tenant?.ownerId}. Members: ${result.data.map(u => `${u.email}(profile:${!!(u as any).profile},role:${(u as any).role?.name})`).join(', ')}`);
